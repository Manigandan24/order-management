...
public class OrderManagementTest {
	...
	public void newOrderTest() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.post("/orders/1")
					.content(iJosn).headers(httpHeaders)
	...
	public void getOrdersListTest() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.post("/orders/2")
				.content(iJosn1).headers(httpHeaders)
	 			.contentType(MediaType.APPLICATION_JSON)
	 			.accept(MediaType.APPLICATION_JSON))
				.andExpect(MockMvcResultMatchers.status().isOk());
		...
		//assertArrayEquals(mapper.readValue(result.getResponse().getContentAsString(), Orders[].class),orders);
